...
func (fe *fieldExtract) Extract(s string) error {
	...
	if leftoverText != "" {
		if loc, err := zoneCacheInstance.LoadLocation(leftoverText); err == nil {
			...
			fe.wanted = fe.wanted.AddAll(dateFields).
				ClearAll(fe.has.Add(fieldTZHour).Add(fieldTZMinute).Add(fieldTZSecond))
		} else {
...
func (fe *fieldExtract) validate() error {
	...
	if (fe.isDB2 && !fe.has.HasAll(db2TimeRequiredFields)) ||
		(fe.has.HasAny(timeRequiredFields) && !fe.has.HasAll(timeRequiredFields)) {
