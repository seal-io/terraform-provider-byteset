...
func newLexemeTerm(lexeme string) (tsTerm, error) {
	if len(lexeme) > 2046 {
		return tsTerm{}, pgerror.Newf(
			pgcode.ProgramLimitExceeded,
			"word is too long (%d bytes, max 2046 bytes)",
			len(lexeme),
		)
	}
...
func TSLexize(config, token string) (lexeme string, stopWord bool, err error) {
...
func DocumentToTSVector(config, input string) (TSVector, error) {
