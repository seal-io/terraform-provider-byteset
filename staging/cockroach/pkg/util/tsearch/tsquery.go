...
func ToTSQuery(config, input string) (TSQuery, error) {
...
func PlainToTSQuery(config, input string) (TSQuery, error) {
...
// performs stopwording and normalization on the tokens, and returns a parsed
// query, interposing the <-> operator between each token.
func PhraseToTSQuery(config, input string) (TSQuery, error) {
...
func cleanupStopword(node *tsNode) (ret *tsNode, lAdd, rAdd int) {
	if node.op == invalid {
		if node.term.lexeme == "" {
			// Found a stop word.
			return nil, 0, 0
