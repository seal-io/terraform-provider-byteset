...
// capacity.
func EncodeNextBytesAscending(b, data []byte) []byte {
	b = append(b, bytesMarker)
	return encodeNextBytesAscendingWithTerminator(b, data, ascendingBytesEscapes.escapedTerm)
}

func encodeNextBytesAscendingWithTerminator(b, data []byte, terminator byte) []byte {
...
func encodeBytesAscendingWithTerminatorAndPrefix(
	b, data []byte, terminator, prefix byte,
) []byte {
	b = append(b, prefix)
...
// lexicographically.
func EncodeBytesDescending(b, data []byte) []byte {
	n := len(b)
...
func DecodeBytesAscendingDeepCopy(b, r []byte) ([]byte, []byte, error) {
...
func DecodeBytesDescending(b, r []byte) ([]byte, []byte, error) {
...
func decodeBytesInternal(
	b, r []byte, e escapes, expectMarker, deepCopy bool,
) ([]byte, []byte, error) {
	if expectMarker {
...
func encodeStringAscendingWithTerminatorAndPrefix(
	b []byte, s string, terminator, prefix byte,
) []byte {
...
func DecodeUnsafeStringAscendingDeepCopy(b, r []byte) ([]byte, string, error) {
...
// buffer.
func DecodeUnsafeStringDescending(b, r []byte) ([]byte, string, error) {
...
func decodeTime(b []byte) (r []byte, sec, nsec int64, err error) {
	if PeekType(b) != Time {
...
func DecodeGeoAscending(b []byte, so *geopb.SpatialObject) ([]byte, error) {
	...
	var pbBytes []byte
	b, pbBytes, err = decodeBytesInternal(
		b,
		pbBytes,
		ascendingGeoEscapes,
		false, /* expectMarker */
...
// buffer, and returns the final buffer.
func EncodeUntaggedBytesValue(appendTo, data []byte) []byte {
...
// `b` or `b[typeOffset:]`.
func DecodeValueTag(b []byte) (typeOffset, dataOffset int, colID uint32, typ Type, err error) {
...
func DecodeBytesValue(b []byte) (remaining, data []byte, err error) {
...
// entire tag.
func PeekValueLength(b []byte) (typeOffset, length int, err error) {
	if len(b) == 0 {
		return 0, 0, nil
	}
	var dataOffset int
