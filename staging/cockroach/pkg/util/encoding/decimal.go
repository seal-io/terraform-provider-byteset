...
func encodeVarExpNumber(tag byte, expAscending bool, exp uint64, m, encInto []byte) []byte {
...
func encodeSmallNumber(negative bool, e int, m, encInto []byte) []byte {
	if negative {
		onesComplement(m)
...
func encodeLargeNumber(negative bool, e int, m, encInto []byte) []byte {
	if negative {
		onesComplement(m)
...
//
// Returns the length-adjusted buffer.
func encodeMediumNumber(negative bool, e int, m, encInto []byte) []byte {
	l := 1 + len(m)
	if len(encInto) < l+1 {
		panic("buffer too short")
	}
...
// decimal from buf.
func DecodeDecimalAscending(buf, tmp []byte) ([]byte, apd.Decimal, error) {
	return decodeDecimal(buf, tmp, false)
}

// DecodeDecimalDescending decodes decimals encoded with EncodeDecimalDescending.
func DecodeDecimalDescending(buf, tmp []byte) ([]byte, apd.Decimal, error) {
	return decodeDecimal(buf, tmp, true)
}

func decodeDecimal(buf, tmp []byte, invert bool) ([]byte, apd.Decimal, error) {
...
// exponent E.
func makeDecimalFromMandE(negative bool, e int, m, tmp []byte) (apd.Decimal, error) {
	if len(m) == 0 {
...
func decodeSmallNumber(
	negative bool, buf, tmp []byte,
) (e int, m, rest, newTmp []byte, err error) {
	var ex uint64
	var r []byte
	if negative {
...
func DecodeNonsortingDecimal(buf, tmp []byte) (apd.Decimal, error) {
	var dec apd.Decimal
...
func DecodeIntoNonsortingDecimal(dec *apd.Decimal, buf, tmp []byte) error {
	*dec = apd.Decimal{}
	switch buf[0] {
	case decimalNaN:
		dec.Form = apd.NaN
