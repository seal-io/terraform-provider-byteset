...
var (
	emptyJSONObject = jsonObject(nil)
	emptyJSONArray  = jsonArray(nil)
)
...
func cmpJSONTypes(a, b Type) int {
	if b > a {
		return -1
	}
	if b < a {
...
var errTrailingCharacters = pgerror.WithCandidateCode(
	errors.New("trailing characters after JSON document"),
...
var errCannotSetPathInScalar = pgerror.WithCandidateCode(
	errors.New("cannot set path in scalar"),
...
var errCannotReplaceExistingKey = pgerror.WithCandidateCode(
	errors.New("cannot replace existing key"),
	pgcode.InvalidParameterValue,
)
...
var (
	errCannotDeleteFromScalar = pgerror.WithCandidateCode(
		errors.New("cannot delete from scalar"),
		pgcode.InvalidParameterValue,
	)
	errCannotDeleteFromObject = pgerror.WithCandidateCode(
		errors.New("cannot delete from object using integer index"),
		pgcode.InvalidParameterValue,
	)
)
...
var errInvalidConcat = pgerror.WithCandidateCode(
	errors.New("invalid concatenation of jsonb objects"),
	pgcode.InvalidParameterValue,
)
...
var errCannotDeletePathInScalar = pgerror.WithCandidateCode(
	errors.New("cannot delete path in scalar"),
	pgcode.InvalidParameterValue,
)
