...
func OverrideWindowDef(base *WindowDef, override WindowDef) (WindowDef, error) {
	...
	if len(override.Partitions) > 0 {
		return WindowDef{}, pgerror.Newf(
			pgcode.Windowing,
			"cannot override PARTITION BY clause of window %q",
			base.Name,
		)
	}
	...
	if len(base.OrderBy) > 0 {
		if len(override.OrderBy) > 0 {
			return WindowDef{}, pgerror.Newf(
				pgcode.Windowing,
				"cannot override ORDER BY clause of window %q",
				base.Name,
			)
		}
	...
	if base.Frame != nil {
		return WindowDef{}, pgerror.Newf(
			pgcode.Windowing,
			"cannot copy window %q because it has a frame clause",
			base.Name,
		)
	}
