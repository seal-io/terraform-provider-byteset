...
func (expr *IndirectionExpr) TypeCheck(
	...
	default:
		return nil, pgerror.Newf(
			pgcode.DatatypeMismatch,
			"cannot subscript type %s because it is not an array or json object",
			typ,
		)
	}
	return expr, nil
...
var (
	...
	errInvalidDefaultUsage = pgerror.New(
		pgcode.Syntax,
		"DEFAULT can only appear in a VALUES list within INSERT or on the right side of a SET",
	)
...
func (expr *FuncExpr) TypeCheck(
	...
	if semaCtx != nil {
		...
		defer func(semaCtx *SemaContext, prevFunc, prevWindow bool) {
	...
	if len(s.overloadIdxs) == 0 {
		return nil, pgerror.Newf(
			...
			"unknown signature: %s",
			getFuncSig(expr, s.typedExprs, desired),
		)
	}
...
func (expr *RangeCond) TypeCheck(
	...
	leftFromTyped, fromTyped, _, _, err := typeCheckComparisonOp(
		ctx,
		semaCtx,
	...
	leftToTyped, toTyped, _, _, err := typeCheckComparisonOp(
		ctx,
		semaCtx,
	...
	_, _, _, _, err = typeCheckComparisonOp(
		ctx,
		semaCtx,
		treecmp.MakeComparisonOperator(treecmp.LT),
		expr.From,
...
) (_, _ TypedExpr, _ *CmpOp, alwaysNull bool, _ error) {
	// Parentheses are semantically unimportant and can be removed/replaced
	// with its nested expression in our plan. This makes type checking cleaner.
...
func typeCheckSplitExprs(
	exprs []Expr,
) (constIdxs, placeholderIdxs, resolvableIdxs intsets.Fast) {
...
func typeCheckTupleComparison(
	...
	for elemIdx := range left.Exprs {
		...
		leftSubExprTyped, rightSubExprTyped, _, _, err := typeCheckComparisonOp(
			ctx,
			semaCtx,
			op,
			leftSubExpr,
...
func typeCheckSameTypedTupleExprs(
	...
	for elemIdx := range first.Exprs {
		...
		if err != nil {
			return nil, nil, pgerror.Wrapf(
				err,
				pgcode.DatatypeMismatch,
				"tuples %s are not the same type",
				Exprs(exprs),
			)
		}
