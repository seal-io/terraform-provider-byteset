...
func CompareDecimals(d, v *apd.Decimal) int {
...
func NewDCollatedString(
	contents, locale string, env *CollationEnvironment,
...
var (
	dIPv4min = ipaddr.Addr(uint128.FromBytes([]byte(ipaddr.ParseIP("0.0.0.0"))))
	dIPv4max = ipaddr.Addr(uint128.FromBytes([]byte(ipaddr.ParseIP("255.255.255.255"))))
	dIPv6min = ipaddr.Addr(uint128.FromBytes([]byte(ipaddr.ParseIP("::"))))
	dIPv6max = ipaddr.Addr(uint128.FromBytes([]byte(ipaddr.ParseIP("ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff"))))
)
...
// is not supported.
func compareTimestamps(ctx CompareContext, l, r Datum) (int, error) {
...
var errNonHomogeneousArray = pgerror.New(
